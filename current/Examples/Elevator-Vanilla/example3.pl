...
prim_fluent(temp).               /* the temperature of the elevator */
prim_fluent(fan).                /* the fan is on or off */
prim_fluent(alarm).              /* the smoke alarm is on or off */
...
initially(temp,2).
initially(fan,off).
initially(light(N),off).   /* all lights off initially */
initially(alarm,off).
