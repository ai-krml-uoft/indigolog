...
initialize(env_manager) :-
	...
	tcp_socket(Socket),
	assert(em_socket(Socket)),
    server_port(PortAddress),
    bind(em_socket, PortAddress),
    ...
    start_env(LEnv, PortAddress), !, % Start each of the devices used (LEnv)
...
start_env_cycle(thread) :-
	thread_create(catch(em_cycle_thread,E,
		...
		;
		 	logging(error,['EM Thread Error:',E])
		)),_,[alias(em_thread)]).
...
em_cycle_thread :-	 % if em_one_cycle/1 has nothing to wait, then just terminate
	 	logging(system(5),'(EM) EM cycle finished, no more streams to wait for...').
...
start_env([Env|LEnv], Address) :-
	...
	call_to_exec(unix, Command, Command2), % Select right command for exec
		logging(system(5), ['(EM) Command to initialize device ', Env, ': ', Command2]),
